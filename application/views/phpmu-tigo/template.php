	...
	<div class="boxed">
		<div class="content">
			<div class="wrapper">
				...
				<?php
				echo "<select class='visible-xs' style='width:100%; padding:5px 0px' name='' onchange=\"document.location.href=this.value\">
					<option value='#'>- Pilih Kategori -</option>";
				$kategori = $this->model_app->view('rb_kategori_produk');
				foreach ($kategori->result_array() as $rows) {
					if ($this->uri->segment(3) == $rows['kategori_seo']) {
						$select = 'selected';
					} else {
						$select = '';
					}
					...
					if ($sub_kategori->num_rows() >= 1) {
						echo "<option value='" . base_url() . "produk/kategori/$rows[kategori_seo]' $select> $rows[nama_kategori]</option>";
						// foreach ($sub_kategori->result_array() as $row) { 
						// 	if ($this->uri->segment(3)==$row['kategori_seo_sub']){ $select = 'selected'; }else{ $select = ''; }
						// 	echo "<option value='".base_url()."produk/subkategori/$row[kategori_seo_sub]' $select> - $row[nama_kategori_sub]</option>";
						// }
					} else {
						echo "<option value='" . base_url() . "produk/kategori/$rows[kategori_seo]' $select> $rows[nama_kategori]</option>";
					}
				...
				if ($this->uri->segment(1) == '' or $this->uri->segment(1) == 'main') {
					echo "
						<div class='form_control'>
							<span class='the-title'><h2>MULAI BERBELANJA</h2></span>
						</div>";
				}
				?>
