...
class Model_reseller extends CI_model{
    ...
    function penjualan($id_reseller){
        return $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_produk=b.id_produk
                                    JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller'AND id_penjual='".$id_reseller."' AND c.proses='1'");
    }
    ...
    function produk_perkategori($id_produk){
        return $this->db->query("SELECT * FROM `rb_produk`ORDER BY waktu_input DESC LIMIT 12");
    }
}