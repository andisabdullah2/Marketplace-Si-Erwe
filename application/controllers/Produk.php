...
class Produk extends CI_Controller {
	...
	function index(){
		...
		if (is_numeric($dari)) {
			if ($this->input->post('kata')){
				...
				$kategori = cetak($this->input->post('cari_kategori'));
				$rt = cetak($this->input->post('cari_rt'));
				$rw = cetak($this->input->post('cari_rw'));
				$data['record'] = $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a 
					LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller 
					LEFT JOIN rb_kota c ON b.kota_id=c.kota_id 
					where c.provinsi_id like '".cetak($this->input->post('cari_rt'))."%' AND c.kota_id like '".cetak($this->input->post('cari_rw'))."%' AND a.id_kategori_produk like '".cetak($this->input->post('cari_kategori'))."%' AND a.id_reseller!='0' AND a.id_produk_perusahaan='0' AND a.nama_produk LIKE '%".cetak($this->input->post('kata'))."%' ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			} else {
				$data['title'] = title();
				...
				$data['record'] = $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a 
					LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller 
					LEFT JOIN rb_kota c ON b.kota_id=c.kota_id 
					where c.provinsi_id like '".cetak($this->input->post('cari_rt'))."%' AND c.kota_id like '".cetak($this->input->post('cari_rw'))."%' AND a.id_kategori_produk like '".cetak($this->input->post('cari_kategori'))."%' AND a.id_reseller!='0' AND a.id_produk_perusahaan='0'  ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			}
	...
	function kurirdata(){
		...
		$dari=$iden['kota_id'];
		$berat=($this->input->get('berat') > 0 ? $this->input->get('berat') : 1);
